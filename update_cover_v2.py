...
def crop_border(image):
    ...
    if float(width)/height < 1.35 and float(width)/height > 0.65:
        ...
        # so for the second pass, use a higher threshold and gamma correction so the view centers around the subject 
        gamma = 0.5
        ...
        window = 50
        pixel_data = moving_average(pixel_data, window)
        pos_data = pos_data[window-1:]
        ...
        adjvalues = pixel_data[1:]/dydx 
        max_index = np.argmax(adjvalues) + window-1
        best_coord = (max_index, best_coord[1])

        fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(10, 4))
        axes[0].plot(pos_data, pixel_data, color='red', linewidth=3)
        axes[0].set_xlabel('x offset')
        axes[0].set_ylabel('perimeter value')
        axes[1].plot(pos_data[1:], dydx, color='blue', linewidth=3)
        axes[1].set_xlabel('x offset')
        axes[1].set_ylabel('derivative value')
        axes[2].plot(pos_data[1:], adjvalues, color='green', linewidth=3)
        axes[2].set_xlabel('x offset')
